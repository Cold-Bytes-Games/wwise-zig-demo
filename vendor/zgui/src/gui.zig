...
fn zguiMemFree(maybe_ptr: ?*anyopaque, _: ?*anyopaque) callconv(.C) void {
    if (maybe_ptr) |ptr| {
        ...
        if (mem_allocations != null) {
            ...
            const mem = @as([*]align(mem_alignment) u8, @ptrCast(@alignCast(mem_alignment, ptr)))[0..size];
...
pub const io = struct {
    ...
    pub fn addFontFromMemoryWithConfig(
        ...
        return zguiIoAddFontFromMemoryWithConfig(fontdata.ptr, @as(i32, @intCast(fontdata.len)), size_pixels, if (config) |c| &c else null, ranges);
    }
