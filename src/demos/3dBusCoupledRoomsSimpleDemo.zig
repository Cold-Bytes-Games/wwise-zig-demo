...
pub fn init(self: *Self, allocator: std.mem.Allocator, demo_state: *root.DemoState) !void {
    ...
    try AK.SoundEngine.registerGameObjWithName(allocator, Room1Obj, "Portal");
    ...
    try AK.SoundEngine.setGameObjectAuxSendValues(allocator, Room1Obj, &.{
        .{
            .listener_id = ListenerObj,
            .aux_bus_id = ID.AUX_BUSSES.LISTENERENV,
            .control_value = 1.0,
        },
    });
...
pub fn onUI(self: *Self, demo_state: *root.DemoState) !void {
    ...
    if (zgui.begin("3d Bus - 3D Portal and Standard Room", .{ .popen = &self.is_visible, .flags = .{} })) {
